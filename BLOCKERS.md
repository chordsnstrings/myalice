...
| BLK-4 | P3 | WhatsApp Cloud API | Meta WABA + phone number ID + permanent token + app secret + webhook verify token (`WHATSAPP_*`, `META_APP_SECRET`). Connector + signed webhook are built. |
| BLK-4b | P3 | Messenger / Instagram | A Meta app + Page access token(s) + verify tokens (`MESSENGER_*`, `INSTAGRAM_*`, shared `META_APP_SECRET`). Connectors + signed webhooks are built; subscribe the page/IG account to the deployed webhook URLs. |
